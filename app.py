...
def category_page(genre):
    ...
    if genre_lower == 'all':
        ...
        filtered = data[data['genres'].str.lower().str.split().apply(lambda g: genre_lower in g if isinstance(g,list) else False)]

    movies = filtered.to_dict(orient='records')

    # TMDb API info
    TMDB_API_KEY = '4a1cec1f5a88caebd4b3390d00560d8e'
    TMDB_SEARCH_URL = 'https://api.themoviedb.org/3/search/movie'
    TMDB_DETAIL_URL = 'https://api.themoviedb.org/3/movie/'

    for movie in movies:
        poster_url = 'https://via.placeholder.com/240x360?text=No+Image'
        try:
            # Search TMDb for movie ID by title
            resp = requests.get(TMDB_SEARCH_URL, params={'api_key': TMDB_API_KEY, 'query': movie['movie_title']})
            resp.raise_for_status()
            results = resp.json().get('results')
            if results:
                movie_id = results[0]['id']
                # Get movie details including poster path
                detail_resp = requests.get(f"{TMDB_DETAIL_URL}{movie_id}", params={'api_key': TMDB_API_KEY})
            ...
            print(f"TMDb API error for '{movie['movie_title']}': {e}")
            # fallback poster_url remains

        movie['poster_url'] = poster_url

    return render_template('category.html', genre=genre, movies=movies)
